...
using UnityEngine;
    ...
    public class ArcadeEngineAudio : MonoBehaviour
    {
        ...
        void Awake()
        {
            GoKart = GetComponentInParent<GoKart>();
        }
        ...
        void Update()
        {
            ...
            if (GoKart != null)
            {
                kartSpeed = GoKart.LocalSpeed();
                Drift.volume = GoKart.IsDrifting && GoKart.GroundPercent > 0.0f ? GoKart.Rigidbody.velocity.magnitude / GoKart.GetMaxSpeed() : 0.0f;
            }
